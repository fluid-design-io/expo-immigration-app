...
**Account Holder**:
The person who signs in and manages filings; may be the applicant themselves or someone acting on behalf of dependents. Distinguished from Applicant because the human using the app is not always the only person being filed for.
...
**A-Number**:
The Alien Registration Number — a unique identifier USCIS assigns to a person that stays the same across every one of their filings, cards, and notices. It identifies the *person*, not any single application; on newer cards it is printed as the "USCIS Number."
...
**Filing**:
A single completed form prepared for one applicant — the unit of work the app guides from blank to ready-to-submit. A Filing becomes a Case only after USCIS receives it.
_Avoid_: Submission, Application instance, Case, Form

**Renewal**:
A filing to obtain a fresh card replacing one that is expiring or has expired, where the underlying status or eligibility continues unchanged. Contrast with Replacement.
_Avoid_: Reapplication, Extension

**Replacement**:
A filing to obtain a new card because the existing one was lost, stolen, damaged, or contains an error — driven by a problem with the card, not by its expiry. Contrast with Renewal.
...
_Avoid_: New application, First filing

**Form Edition**:
The dated version of an official USCIS form; USCIS rejects filings prepared on an outdated edition, so the current edition must always be used.
...
**Case**:
An in-progress matter at USCIS, created the moment USCIS receives a filing and tracked over time by its receipt number. A Case exists only after submission — before that it is a Filing.
_Avoid_: Ticket, Matter

**Receipt Number**:
The identifier USCIS assigns to a received filing (three letters — e.g. IOE, MSC, EAC, WAC, LIN, SRC — followed by ten digits) used to look up a case's status. It identifies a *case*, not a person, unlike the A-Number; USCIS sometimes calls this the case number, but we standardize on Receipt Number.
_Avoid_: Case Number, Confirmation number, Tracking number

**Receipt Notice**:
The notice USCIS issues (Form I-797) confirming a filing was received and bearing the receipt number.
...
**Applicant Profile**:
The persistent set of an applicant's personal, address, and eligibility information that is reused to pre-fill future filings — the asset that makes a second filing near one-tap.
_Avoid_: Account, User record, Form data

**Autofill**:
Populating a new filing's fields from the applicant profile and prior filings rather than re-asking the applicant.
