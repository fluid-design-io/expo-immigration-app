...
**Feasibility**: **HIGH** — verified tooling path (Expo + EAS, Drizzle + Postgres on Railway, Railway cron → Expo push) confirmed against primary vendor docs during research.

**Architecture Notes**
- **Client**: Expo SDK 56 / RN 0.85 / React 19, Expo Router native tabs, HeroUI Native Pro, Uniwind/Tailwind, Reanimated. **Dev build required** (push notifications + secure store don't run in Expo Go).
- **Backend**: Railway — Postgres + Drizzle (`pg` driver), Hono API, Better Auth (self-hosted → we own PII). Documents in object storage with server-signed URLs.
- **Reminders**: Railway cron (daily) → server POST to Expo Push (`exp.host/--/api/v2/push/send`), batch ≤100, then fetch receipts ~15 min later to prune dead tokens.
- **Data model**: durable `users` + `applicant_profile` + reusable `documents` vault, separated from per-filing `applications` (JSONB form answers, **not EAV**), with self-referential `renews_application_id` for renewal history.
- **Secrets**: all PII-access keys server-side; nothing sensitive in the client bundle (verified: client bundle is always public).

**Technical Risks**

| Risk | Likelihood | Mitigation |
|------|------------|------------|
| **PII / compliance** (passports, A-numbers, EAD) | H | Encrypt sensitive fields; minimize storage; secure-store tokens; deletion/export; OWASP MASVS; security-reviewer + `/santa-loop` on auth/PII |
| **UPL** (drifting into legal advice) | M | Disclaimers; information + form-assist only; validate scope with an immigration attorney |
| **USCIS case-status integration uncertainty** | M | Spike the developer API early; fall back to manual receipt-number entry |
| **App-store rejection** | M | Privacy policy + accurate data-safety disclosures; in-app account deletion; never imply government affiliation |
| **Form-logic accuracy** (wrong field → rejected filing) | M | Encode against official I-90/I-765 instructions; expert review; clear "verify before submitting" step |
...
| 2 | Backend core | Railway: Postgres + Drizzle schema, Hono API, Better Auth, signed uploads | pending | with 1 | - | - |
...
| 5 | Calendar + reminders | Deadlines UI, Railway cron, Expo push pipeline | pending | with 4 | 2 | - |
...
**Phase 2: Backend core** — Goal: secure API + DB on Railway. Scope: Drizzle schema + migrations, Hono endpoints, auth middleware, signed upload/download. Success: protected endpoint works; a document uploads via signed URL and re-downloads.

**Phase 3: Filing wizard** — Goal: complete an I-90 or I-765 renewal end-to-end. Scope: JSONB-driven multi-step form, autosave drafts, profile/vault pre-fill, review/export. Success: a renewal completes with most fields pre-filled from profile.

**Phase 4: Tracker** — Goal: see case status in one place. Scope: receipt entry, status + history, USCIS API spike. Success: a case shows current status + history.

**Phase 5: Calendar + reminders** — Goal: never miss a renewal window. Scope: deadlines from documents/cases, calendar UI, Railway cron, Expo push. Success: a device receives a scheduled due-date reminder.

**Phase 6: Harden + ship** — Goal: safe and in the stores. Scope: 80% coverage, a11y, security/PII review, account deletion, store assets, EAS submit + Update. Success: approved on both stores; OTA channel live.

### Parallelism Notes

Phases 1 and 2 run together (frontend skeleton while backend stands up). Phases 4 and 5 run together (both depend only on Phase 2's API and don't touch each other). Phase 3 needs both 1 and 2. Phase 6 gates on all feature phases.
...
| Backend | Railway (Postgres + Hono + Drizzle) | Supabase, Firebase, Expo API routes | User choice; verified Railway+Drizzle path; self-host auth → own PII |
| Auth | Better Auth (self-hosted) | Clerk, Lucia | Owns PII (sensitive immigration data); Expo support |
| Dynamic form storage | JSONB on `applications` | EAV | EAV is a Postgres anti-pattern; JSONB fits variable per-type forms |
...
**Technical Context** — Expo + EAS + Railway (Postgres/Drizzle/cron) + Expo Push is a verified, high-feasibility stack. Primary constraints are PII/compliance and the depth/accuracy of encoded form logic, not the platform.
...
*Generated: 2026-06-22 · Status: DRAFT — needs validation*
